...
const BlockTable = () => {
	...
	const getBlockList = async () => {
		...
		const trxCountEdp = "https://api.tzkt.io/v1/operations/transactions/";
		const blockRes = await axios.get(blockEdp);
		const blocks = blockRes.data;
		blocks.sort(function (a, b) { return b.level - a.level });
		const trxRes = await axios.get(trxCountEdp);
		const trxs = trxRes.data;
		blocks.map(block => {
			const trx = trxs.filter(trx => trx.level == block.level);
			block.trxCount = trx.length;
		});
		setBlockList(blocks);
	};
